...
void solution(){
  ...
  long int i,c,d,l,minv,maxv;
  
    cin >> c>>d>>l;
    
    maxv = 4*(c+d); // if all cats and dogs are on the ground
    
    minv = 4*d; //if all cats can sit on top of the dogs i.e. c<=2*d  
    if (c>2*d)
    {
      minv += 4*(c-2*d);  // if all cats can't sit on top of the dogs. So (c-2*d) number of cats will be on the ground
    }
    
    if ( (l%4==0) and (l>=minv) and (l<=maxv) )
    {
      cout << "yes" << "\n";
    }
